...
function SWEP:PrimaryAttack()
	...
	if ( IsValid( ent ) && self:Clip1() >= need && ( ent:IsPlayer() || ent:IsNPC() ) && ent:Health() < 100 ) then
		...
		if self.Owner:GetVelocity():Length() > 1 and ent:GetVelocity():Length() > 1 then
	
			self:ChatPrint("Both you and the target need to be standing still in order for the medkit to work.")
		...
		if DarkRP then
			if need > 0 then
				self.GetOwner():addMoney(need*MoneyPerHP)
			end
		end
...
function SWEP:SecondaryAttack()
	...
	if ( IsValid( ent ) && self:Clip1() >= need && ent:Health() < ent:GetMaxHealth() ) then
		...
		if self.Owner:GetVelocity():Length() > 1 and ent:GetVelocity():Length() > 1 then
			...
			self:ChatPrint("You need to be standing still in order for the medkit to work.")
			self.Owner:EmitSound( DenySound )
			self:SetNextPrimaryFire( CurTime() + 1 )
